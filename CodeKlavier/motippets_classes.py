...
class Motippets(object):
    ...
    def __init__(self, mapping, noteonid, noteoffid, mid_low, mid_hi, playedlimit=1):
        ...
        for c in self._allConditional_motifs:
            ...
            if (
                bottom_note < self._pianosections[0]
                and top_note > self._pianosections[0]
            ) or (
                bottom_note < self._pianosections[1]
        ...
        self._interval = 0
        self._lastTremolo = {"hi": 0, "mid": 0, "low": 0}
        ...
        if message[0] == self.noteonid:
                    ...
                    for motif in self._miniMotifsLow:
                        if self._miniMotifsLow[motif]["type"] == "mel":
                            ...
                            for m in self._miniMotifsLow:
                                if m != motif:
                                    other_minis_count.append(
                                        self._miniMotifsLow[m]["count"]
                                    )
                            ...
                            if (
                                self._miniMotifsLow[motif]["played"]
                                and np_others.sum() == 0
                            ):
                    ...
                    for motif in self._miniMotifsMid:
                        if self._miniMotifsMid[motif]["type"] == "mel":
                            self._miniMotifsMid[motif]["played"] = self.compare_motif(
                                None, motif, mini_motifs_mel.get(motif), note, section
                            )
                            ...
                            if (
                                self._miniMotifsMid[motif]["played"]
                                and np_others.sum() == 0
                            ):
                                self.mapscheme.miniSnippets(motif, section)
                ...
                if note > self._pianosections[1]:
                    ...
                    for motif in self._miniMotifsHi:
                        if self._miniMotifsHi[motif]["type"] == "mel":
                            ...
                            if (
                                self._miniMotifsHi[motif]["played"]
                                and np_others.sum() == 0
                            ):
                                self.mapscheme.miniSnippets(motif, section)
            ...
            ### TREMOLO
                ...
                if note <= self._pianosections[0]:
                    ...
                    if (
                        self.count_notes(self._memory, False) == 8
                        and len(self._memory) > 3
                    ):
                        self.tremolo_value(
                            [self._memory[2], self._memory[3]],
                            "low",
                            ck_deltatime_low,
                            0.15,
                            target,
            ...
            ### FULL REGISTER (used for Main Motifs)
                ...
                for motif in self._allMainMotifs:
                    if self._motifsCount[motif]["type"] == "chord":
                        ...
                        if (
                            self._motifsCount[motif]["played"]
                            and self._motifsCount[motif]["count"] < self._playedlimt
                        ):
                            self.mapscheme.snippets(motif)
                            self._motifsCount[motif]["count"] += 1
                    else:
                        self._motifsCount[motif]["played"] = self.compare_motif(
                            None, motif, motifs_mel.get(motif), note
                        )
                        if (
                            self._motifsCount[motif]["played"]
                            and self._motifsCount[motif]["count"] < self._playedlimt
                            ...
                            for register in self._memoryCond:
                                if self._conditionalCount[motif]["type"] == "chord":
                                    self._conditionalCount[motif][
                                        ...
                                        self._memoryCond[register],
                                        motif,
                                        conditional_motifs.get(motif),
                                        note,
                                        deltatime=self._memoryCondDeltas[
                                            "delta" + register
                                        ],
                                        pianosection=register,
                                        debug=False,
                                    )
                                else:
                                    self._conditionalCount[motif][
                                        ...
                                        self._memoryCond[register],
                                        motif,
                                        conditional_motifs_mel.get(motif),
                                        note,
                                        pianosection=register,
                                        debug=False,
                                    )
                        ...
                        for register in self._memoryCond:
                            if self._conditionalCount[result]["type"] == "mel":
                                ...
                                self._conditionalCount[result][
                                    ...
                                    self._memoryCond[register],
                                    result,
                                    conditional_results_motifs.get(result),
                                    note,
                                    deltatime=self._memoryCondDeltas[
                                        "delta" + register
                                    ],
                            ...
                            if (
                                self._conditionalCount[result]["played"]
                                and self._resultCounter == 0
                            ):
            ...
            ### CONDITIONAL RANGE
                ...
                section == "conditional_range"
            ):  # parses only MIDI for the conditional which looks at the range being played
                self.memorize(
                    note, 999, debugname="Range conditional memory", debug=False
                )

                self.get_range(self._memory, self._timer, debug=False)

    def memorize(
        self, midinote, length, debug=False, debugname="Motippets", conditional="off"
    ):
    ...
    def memorizeCond(
        self, midinote, length, debug=False, pianosection=None, delta=False
    ):
        """Store the incoming midi notes by appending to the memory array.
        ...
        if not delta:
            ...
            if len(self._memoryCond[pianosection]) > length:
                self._memoryCond[pianosection] = self._memoryCond[pianosection][
                    -length:
                ]
            ...
            if len(self._memoryCondDeltas[pianosection]) > length:
                self._memoryCondDeltas[pianosection] = self._memoryCondDeltas[
                    pianosection
                ][-length:]
            ...
            if debug is True:
                print(
                    pianosection,
                    ",".join(map(str, self._memoryCondDeltas[pianosection])),
                )
    ...
    def count_notes(self, array, debug=False):
        ...
        if len(array) > 7:
            ...
            if debug:
                print(
                    "array is " + str(array),
                    "repeated notes1: " + str(count1),
                    "repeated notes2: " + str(count2),
                )
    ...
    def compare_motif(
        self, array, motif_name, motif, note, pianosection=None, debug=False
    ):
    ...
    def compare_chordal_motif(
        ...
        if len(played) >= len(motif):
            ...
            sum_played = reduce((lambda total, sumnotes: total + sumnotes), played)
            dif_delta = reduce(
                (lambda total, sumtimes: (total + sumtimes) / len(deltas)), deltas
            )

            if sum_motif == sum_played and (
                dif_delta < deltatolerance and dif_delta > 0
            ):
                compare = True
                if pianosection == "low":
    ...
    def tremolo_value(
        self, notes, pianosection, deltatime, deltatolerance, target=None, debug=False
    ):
        ...
        if deltatime < deltatolerance:
            ...
            interval_reduce = reduce(
                (lambda total, sumnotes: total - sumnotes), self._intervalsArray
            )
            ...
            if interval_reduce != 0 and interval > 0:
                ...
                if self._lastTremolo[pianosection] != interval:
                    self._lastTremolo[pianosection] = interval
